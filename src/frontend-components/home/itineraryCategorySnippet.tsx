...
const ItineraryCategorySnippet = (props)=>{
    ...
    return (
        <>
            <div onClick={()=>navigate(`/details/${props.data.id}/${urlTourName}`)}>
                <div className="w-full rounded-md mx-0 md:mx-1 lg:mx-1">
                    <div className="trip_by_destination_list_img rounded-lg bg-cover h-56 bg-center bg-no-repeat" style={{
                        backgroundImage: `url(${imgPath}${props.data.thumbnail})`
                    }}>
                        <div className='flex items-center justify-center'>
                            ...
                            <div className="package_duration absolute top-5 right-0 bg-black py-1 px-4">
                                <span className="text-white">{props.data.totalDays-1} Nights {props.data.totalDays} Days, ₹ 
                                <span className="text-lg font-semibold text-orange-500 lg:text-sm xl:text-xl 2xl:text-lg">{_.round((4*pax[1] + pax[2] + Number(paxTC[1]))/8)}</span></span>
                            </div>
                            <div
                ...
                {/* <p className="text-center text-xl font-medium">{_.startCase(_.toLower(props.data.tourName))}</p> */}
                <p className="text-center text-xl font-medium">{props.data.tourName}</p>
            </div>
        </>
    )
...
export default ItineraryCategorySnippet;