...
const hotelFilter = ({selectFilterType, selectFilterModel})=>{
    const server = api.API_URL;
    const [resDataType, setResDataType] = useState('');
    const [resDataCar, setResDataCar] = useState('');
    const fetchData = ()=>{
       
         const res = axios.get(`${server}super-transport-master/front/get-bus-filter/`).then((response)=> {
    ...
    return (
        <>
           <div className="review_sidebar_wrap p-8 rounded-lg bg-white border-slate-200 border-solid border shadow-lg">
                              ...
                              <div className="mb-6">
                                 <ul>
                                    <li><label className="text-base font-bold text-left pb-3 block">Bus Type</label></li>
                              ...
                              <div>
                                 <ul>
                                    <li><label className="text-base font-bold text-left pb-3 block">Bus Model</label></li>
                                    { resDataCar.length >0 ?
