    ...
    return (
        ...
        {(allow) ? <div className="my-4">
                            ...
                            <div key={idx} className="shadow-md rounded-lg p-5 m-2 border border-slate-200 border-solid" style={{/*borderWidth:'2px'*/}}>
                                {(item.field === 'hotel') ? 
                                    <div className="grid grid-cols-12  pb-2">
                                        <div className="col-span-8">
                                            ...
                                            <p className="text-xs">{season} | {toggle} AC</p>
                                ...
                                {(item.field === 'transport') ?                                    
                                    <div className="grid grid-cols-12 pb-2">
                                        ...
                                        <p className="text-xs">{season} | {toggle} AC | Seats {getTransport(item.id)?.noOfseat}</p>                                     
            ...
            <Slideover.Panel>
                ...
                {(hotel?.length != 0) ? _.take(hotel, hotel?.length).filter(x => [props.fromID,props.toID].includes(x.cityId) && ((acFilter === 'Y') ? (x['minRoomRent' + `${(season === 'On-season') ? 'Season' : 'OffSeason'}` + 'AC'] != '' ) : (x['minRoomRent' + `${(season === 'On-season') ? 'Season' : 'OffSeason'}` + 'NonAC'] != '' )) && x['is' + mpFilter] === 'Y').map((item, Key) => {
                let seasonCheck = (season === 'On-season') ? 'Season' : 'OffSeason' 
                let toggle = (acFilter === 'Y') ? '' : 'Non'
                let ACval = (acFilter === 'Y') ? true : false
                let on_click = () => {setPlan([...plan,{ id: item.id, field: 'hotel', isAC: ACval, mealPlan: 'EP', startTime: null, lastTime: null, others: [item.hotelName] }])}
                if (planAction === 'Replace') {
                    on_click = () => {modifyPlan(currPlanIdx,{ id: item.id, field: 'hotel', isAC: ACval, mealPlan: 'EP', startTime: null, lastTime: null, others: [item.hotelName] })}
                }
                ...
                <div className="grid grid-cols-12">
                    ...
                    <b><span className="text-2xl">₹</span><span className="text-4xl">{item['minRoomRent' + seasonCheck + toggle + 'AC']}</span></b><br />
                    {season} | {toggle} AC
                    </div>
                </div>
                </div>
            ...
            <Slideover.Panel>
                ...
                let disable = '', on_click = () => {setPlan([...plan,{ id: item.id, field: 'transport', isAC: ACval, startTime: null, lastTime: null, others: [item.transportName] }])}
                if (planAction === 'Replace') {
                    on_click = () => {modifyPlan(currPlanIdx,{ id: item.id, field: 'transport', isAC: ACval, startTime: null, lastTime: null, others: [item.transportName] })}
                }
                // let checkTime = null
                // if (plan.length != 0) {
                ...
                <div className="grid grid-cols-12">
                    ...
                    <div className="p-3 col-span-5 text-end" >
                    <b><span className="text-2xl">₹</span><span className="text-4xl">{item['price' + seasonCheck + toggle + 'AC']}</span></b><br />
                    {season} | {toggle} AC<br />
