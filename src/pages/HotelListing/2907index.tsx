...
import HotelListingSnippet from "../../frontend-components/hotel/hotelListingSnippet";
 ...
 const changeCPage = ()=>{}
   ...
   const fetchData = (value='',limit=200,pageNo=0)=>{
      ...
      const dataPageNo = `&sLimit=${pageNo*limit}`;
       ...
       const res = axios.get(`${server}vendor-hotel-master/front/get-all-hotel/ ${dataStr}${dataLimit}${dataPageNo}`).then((response)=> {
                                          ...
                                          <div className="p-0">
                                             ...
                                             { resData.length >0 ?
                                                _.take(resData, resData.length).map((item, Key) => (
                                                   <>
                                                      <HotelListingSnippet data={item}></HotelListingSnippet>
                                                   </>
                                                ))
                                             :'No Record Found' }
                                          </div>
